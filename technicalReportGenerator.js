...
function generateTechnicalReport(data) {
    ...
    // --- ATUALIZAÇÃO AQUI ---
    const planetOrder = ['sun', 'moon', 'mercury', 'venus', 'mars', 'jupiter', 'saturn', 'uranus', 'neptune', 'pluto', 'chiron', 'north_node', 'south_node'];
    const PLANET_NAMES_MAP = {
        sun: 'Sol', moon: 'Lua', mercury: 'Mercúrio', venus: 'Vênus', mars: 'Marte',
        jupiter: 'Júpiter', saturn: 'Saturno', uranus: 'Urano', neptune: 'Netuno',
        pluto: 'Plutão', chiron: 'Quíron', // --- ADICIONADO ---
    ...
    planetOrder.forEach((planetName, index) => {
        ...
        // Adicionamos uma verificação: Quíron não tem dignidades clássicas.
        if (p.dignities) {
            report += `Condição: ${getConditionString(p.dignities)}\n`;
            if (p.dignities.decan) {
